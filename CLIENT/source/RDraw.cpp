...
RDraw::RDraw()
{
    ...
    m_settings = Mat(m_settings_height, m_settings_width, CV_8UC3, m_overlay_color);
    m_path.resize(m_grid_rows, vector<bool>(m_grid_cols, false));
...
{
    cvui::context("GUI");
    Point m_settings_position = Point(0, 0);
    ...
    // --- Status ---
    text(m_settings, m_settings_position.x, m_settings_position.y, "Status: ");
    text(m_settings, m_settings_position.x + 50, m_settings_position.y, "ONLINE"); // Placeholder for actual status value

    // --- Mode ---
    m_settings_position += cv::Point(0, 35);
    text(m_settings, m_settings_position.x, m_settings_position.y, "Mode");

    // Mode Toggle Button
    string mode_label = m_auto ? "Auto" : "Manual";
    m_settings_position += cv::Point(0, 15);
    if (button(m_settings, m_settings_position.x, m_settings_position.y, 80, 30, mode_label)) {
        m_auto = !m_auto; // Toggle mode state
    }

    // --- Speed ---
    m_settings_position += cv::Point(0, 40);
    text(m_settings, m_settings_position.x, m_settings_position.y, "Speed");

    // Speed Toggle Button
    string speed_label = m_fast ? "Fast" : "Slow";
    m_settings_position += cv::Point(0, 15);
    if (button(m_settings, m_settings_position.x, m_settings_position.y, 80, 30, speed_label)) {
    ...
    bool path_checkbox_changed = checkbox(m_settings, m_settings_position.x, m_settings_position.y, "Edit Path", &m_edit_path);
    m_settings_position += cv::Point(0, 25);
    bool obstacles_checkbox_changed = checkbox(m_settings, m_settings_position.x, m_settings_position.y, "Edit Obstacles", &m_edit_obstacles);
